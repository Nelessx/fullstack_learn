...
app.post("/articles", upload.single("thumbnail"), async (req, res) => {
  try {
    ...
    const newArticle = await new Article({...req.body,thumbnail: cloudinaryResponse.secure_url}).save();
    return res.status(201).json({
...
app.patch("/articles/:id", upload.single("thumbnail"), async (req, res) => {
  try {
    ...
    const updateArticle = await new Article({...req.body,thumbnail: cloudinaryResponse.secure_url}).save();
    return res.status(201).json({
        message: "article created sucessfully",
